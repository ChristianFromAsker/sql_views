...
, IF(tq.referral_help = '' OR tq.referral_help IS NULL
, 'no referral help'
, tq.referral_help
) referral_help
...
LEFT JOIN
cm_deal_questions_t cdq
ON r.deal_question_id = cdq.id
LEFT JOIN
stella_common.cm_template_questions_t tq
ON cdq.template_question_id = tq.id
LEFT JOIN
stella_common.binders_t b
ON r.binder_id = b.binder_id
LEFT JOIN
stella_common.menu_list_t an
ON cdq.answer_id = an.menu_id
LEFT JOIN
stella_common.menu_list_t ga
ON tq.good_answer_id = ga.menu_id
LEFT JOIN
stella_common.menu_list_t rs
ON r.referral_status_id = rs.menu_id
LEFT JOIN
stella_common.menu_list_t rl
ON tq.referral_level_id = rl.menu_id
LEFT JOIN
stella_common.menu_list_t dt
ON tq.due_time_for_referral_id = dt.menu_id
LEFT JOIN
stella_common.cm_question_types_t qt
