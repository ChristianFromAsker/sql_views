...
SELECT
    ...
    , d.analyst_id
    , analyst.uw_initials analyst_uw_initials
...
LEFT JOIN stella_common.risk_types_t rtm ON
    risk_type.risk_type_major_id__risk_types_t  = rtm.risk_type_id
LEFT JOIN stella_common.menu_list_t qd ON
    d.was_quoted_id = qd.menu_id
LEFT JOIN broker_firms_t bf ON
    d.broker_firm_id = bf.broker_firm_id
LEFT JOIN broker_persons_t bp ON
    d.broker_person = bp.ID
LEFT JOIN law_firms_t blf1 ON
    d.buyer_law_firm_1_id = blf1.law_firm_id
LEFT JOIN law_firms_t blf2 ON
    d.buyer_law_firm_2_id = blf2.law_firm_id
LEFT JOIN law_firms_t slf ON
    d.SellerLegalFirm = slf.law_firm_id
LEFT JOIN lawyers_t uc ON
    d.UwCounselPerson1 = uc.lawyer_id
LEFT JOIN stella_common.underwriters_t uw ON
    d.primary_uw = uw.uw_id
LEFT JOIN stella_common.underwriters_t suw ON
    d.secondary_uw = suw.uw_id
...
LEFT JOIN stella_common.jurisdictions_t tlj ON
    d.TargetDomicile = tlj.jurisdiction_id
LEFT JOIN stella_common.jurisdictions_t ic ON
    d.BuyerDomicile = ic.jurisdiction_id
LEFT JOIN stella_common.jurisdictions_t irc ON
